...
class XML_Feed_Generator {
	...
	private function build_xml_content( $category, $products ) {
		...
		foreach ( $products as $product ) {
			$offer = $this->create_offer_element( $xml, $product, $category->slug );
			if ( $offer ) {
	 ...
	 * @param string      $feed_category_slug Category slug for which feed is being generated
	 * @return DOMElement|false
	 */
	private function create_offer_element( $xml, $product, $feed_category_slug ) {
		$offer = $xml->createElement( 'offer' );
		$offer->setAttribute( 'id', $product->ID );
		$offer->setAttribute( 'available', 'true' );
		
		// Product name with category prefix
		$product_name = $this->get_product_name_with_category( $product, $feed_category_slug );
		$offer->appendChild( $xml->createElement( 'name', esc_html( $product_name ) ) );
		
		// Collection ID (feed category slug)
		$offer->appendChild( $xml->createElement( 'collectionId', esc_html( $feed_category_slug ) ) );
		...
		// Category ID
		$offer->appendChild( $xml->createElement( 'categoryId', $feed_category_slug ) );
	 ...
	 * @return string
	 */
	private function get_product_name_with_category( $product, $feed_category_slug ) {
		...
		if ( ! $product_categories || is_wp_error( $product_categories ) ) {
			...
			$category_name = $this->category_names[ $feed_category_slug ] ?? 'Платье';
	...
	public function generate_combined_feed() {
		...
		$limited_counts = array( 'wedding' => 360, 'evening' => 72, 'prom' => 96 );
		...
		$public_path = ABSPATH . 'xml/combined.xml';
	...
	private function build_combined_xml_content( $categories, $products ) {
		...
		foreach ( $products as $product ) {
			...
			$product_categories = get_the_terms( $product->ID, 'dress_category' );
			$feed_category_slug = 'wedding'; // default
			
			if ( $product_categories && ! is_wp_error( $product_categories ) ) {
				$category_slugs = array_map( function( $term ) {
					return $term->slug;
				}, $product_categories );
				$feed_category_slug = $this->get_priority_category( $category_slugs );
			}
			
			$offer = $this->create_offer_element( $xml, $product, $feed_category_slug );
			if ( $offer ) {
				$offers->appendChild( $offer );
			}
		}
