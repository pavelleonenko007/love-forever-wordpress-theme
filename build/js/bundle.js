...
window.addEventListener('fitting:datetimechange', event => {
  const globalFittingDialog = _Dialog__WEBPACK_IMPORTED_MODULE_1__["default"].getDialogsById('globalFittingDialog')?.dialog;
  const singleProductFittingDialog = _Dialog__WEBPACK_IMPORTED_MODULE_1__["default"].getDialogsById('singleProductFittingDialog')?.dialog;
  [globalFittingDialog, singleProductFittingDialog].forEach(dialog => {
    const dialogTimeElement = dialog.querySelector('[data-js-fitting-form-selected-date]');
    if (dialogTimeElement) {
      console.log({
        dialogTimeElement
      });
      dialogTimeElement.textContent = (0,_utils__WEBPACK_IMPORTED_MODULE_2__.formatDateToRussian)(event.detail.datetime);
    }
  });
});
window.addEventListener('fitting:created', event => {
  ...
  const singleProductFittingDialog = _Dialog__WEBPACK_IMPORTED_MODULE_1__["default"].getDialogsById('singleProductFittingDialog')?.dialog;
  [globalFittingDialog, singleProductFittingDialog].forEach(dialog => {
    const closeButtons = dialog.querySelectorAll('[data-js-dialog-close-button]');
    const fieldsContainerElement = dialog.querySelector('.fitting-form__group-body');
...
window.addEventListener('fitting:createerror', event => {
  const errorElements = document.querySelectorAll('[data-js-fitting-form-errors]');
...
class FittingForm {
  ...
  constructor(element) {
    this.root = element;
    this.submitButton = Array.from(document.querySelectorAll('button[type="submit"]')).find(button => button.form === this.root);
  ...
  emitDateTimeUpdate() {
    ...
    window.dispatchEvent(new CustomEvent('fitting:datetimechange', {
      detail: {
        datetime
      }
    }));
  ...
  async updateTimeSlots() {
    ...
    const {
      data,
      error
    } = await (0,_utils__WEBPACK_IMPORTED_MODULE_2__.promiseWrapper)(getAvailableSlotsForDate(this.root.date.value));
    if (error) {
      alert(error);
      return;
    }
  ...
  validateForm() {
    ...
    const submitButton = Array.from(document.querySelectorAll('[data-js-fitting-form-submit-button]')).find(button => button.form === this.root);
    if (submitButton) {
  ...
  onSubmit = async event => {
    ...
    const {
      data,
      error
    } = await (0,_utils__WEBPACK_IMPORTED_MODULE_2__.promiseWrapper)(createFittingRecord(formData));
    this.hasBeenSubmitted = true;
    this.submitButton.disabled = false;
    if (error) {
      window.dispatchEvent(new CustomEvent('fitting:createerror', {
        detail: {
          message: error
        }
      }));
    ...
    window.dispatchEvent(new CustomEvent('fitting:created', {
      detail: {
   ...
   */
  onDialogClose = event => {
    if (!['singleProductFittingDialog', 'globalFittingDialog'].includes(event.detail.dialogId)) {
      return;
    }
    ...
    const {
      dialog
    } = _Dialog__WEBPACK_IMPORTED_MODULE_1__["default"].getDialogsById(event.detail.dialogId);
...
class GlobalFittingForm extends BaseFittingForm {
  ...
  constructor(form) {
    ...
    this.dialogSelectedTime.textContent = (0,_utils__WEBPACK_IMPORTED_MODULE_2__.formatDateToRussian)(`${this.state.date} ${this.state.time}`);
    this.bindEvents();
  }
...
class GlobalFittingFormSimpler extends BaseFittingForm {
  ...
  async updateTimeSlots() {
    ...
    const {
      data,
      error
    } = await (0,_utils__WEBPACK_IMPORTED_MODULE_2__.promiseWrapper)(this.getNewTimeSlots());
    if (error) {
      this.state.success = false;
      this.state.error = error.message;
      this.state.isUpdatingSlots = false;
...
class SingleFittingForm extends BaseFittingForm {
  ...
  async loadTimeSlots() {
    ...
    try {
      ...
      timeSelectControl.dispatchEvent(new Event('change', {
        bubbles: true
      }));
    } catch (error) {
      console.error(error);
...
class EditFittingForm extends BaseFittingForm {
  ...
  async updateTimeSlots() {
    if (this.state.isUpdatingSlots) return;
    this.state.isUpdatingSlots = true;
    await (0,_utils__WEBPACK_IMPORTED_MODULE_2__.wait)(2000);
    const {
      data,
      error
    } = await (0,_utils__WEBPACK_IMPORTED_MODULE_2__.promiseWrapper)(this.getNewTimeSlots());
  ...
  submitForm = async event => {
    ...
    const {
      data,
      error
    } = await (0,_utils__WEBPACK_IMPORTED_MODULE_2__.promiseWrapper)(this.updateFitting());
    this.state.isSubmitting = false;
    if (error) {
      this.state.success = false;
      this.state.error = error.message;
...
class Stories {
  ...
  constructor(element, options = {
    ...
    this.holdDelay = 300;
    this.root = new _splidejs_splide__WEBPACK_IMPORTED_MODULE_1__["default"](this.element, {
      width: '465rem',
      perMove: 1,
      perPage: 1,
      pagination: false,
    ...
    this.element.querySelectorAll(this.selectors.innerSlider).forEach((element, index) => {
      const instance = new _splidejs_splide__WEBPACK_IMPORTED_MODULE_1__["default"](element, {
        perPage: 1,
        perMove: 1,
        type: 'fade',
        autoWidth: true,
  ...
  onRootClick = slide => {
    setTimeout(() => {
      if (this.root.state.is(_splidejs_splide__WEBPACK_IMPORTED_MODULE_1__["default"].STATES.MOVING)) {
        return;
      }
  ...
  onPointerDown = event => {
    ...
    if (event.target.closest(this.selectors.storyCta) || event.target.closest('.splide__pagination__page')) {
      return;
    }
    event.preventDefault();
    event.stopPropagation();
  ...
  openStory(index = 0) {
    if (index >= this.root.length) {
      _Dialog__WEBPACK_IMPORTED_MODULE_2__["default"].getDialogsById('storiesDialog').close();
      return;
    }
    if (index < 0) {
      index = 0;
...
/* harmony import */ var _VideoPlayer__WEBPACK_IMPORTED_MODULE_19__ = __webpack_require__(/*! ./VideoPlayer */ "./src/VideoPlayer.js");
/* harmony import */ var _CardSlider__WEBPACK_IMPORTED_MODULE_20__ = __webpack_require__(/*! ./CardSlider */ "./src/CardSlider.js");
/* harmony import */ var _PlayIfVisibleVideoCollection__WEBPACK_IMPORTED_MODULE_21__ = __webpack_require__(/*! ./PlayIfVisibleVideoCollection */ "./src/PlayIfVisibleVideoCollection.js");
/* harmony import */ var _SaleTimer__WEBPACK_IMPORTED_MODULE_22__ = __webpack_require__(/*! ./SaleTimer */ "./src/SaleTimer.js");
/* harmony import */ var _CustomDatePicker__WEBPACK_IMPORTED_MODULE_23__ = __webpack_require__(/*! ./CustomDatePicker */ "./src/CustomDatePicker.js");
/* harmony import */ var _Stories__WEBPACK_IMPORTED_MODULE_24__ = __webpack_require__(/*! ./Stories */ "./src/Stories.js");
/* harmony import */ var _styles_index_scss__WEBPACK_IMPORTED_MODULE_25__ = __webpack_require__(/*! ./styles/index.scss */ "./src/styles/index.scss");
...
function initPage() {
  ...
  _PlayIfVisibleVideoCollection__WEBPACK_IMPORTED_MODULE_21__["default"].init();
  initHeroPageSliderPagination();
  _CardSlider__WEBPACK_IMPORTED_MODULE_20__["default"].init();
  ...
  _VideoPlayer__WEBPACK_IMPORTED_MODULE_19__["default"].init();
  ...
  _SaleTimer__WEBPACK_IMPORTED_MODULE_22__.SaleTimerCollection.init();
  _CustomDatePicker__WEBPACK_IMPORTED_MODULE_23__["default"].init();
}
...
document.addEventListener('catalog:updated', () => {
  _CardSlider__WEBPACK_IMPORTED_MODULE_20__["default"].destroyAll();
  _CardSlider__WEBPACK_IMPORTED_MODULE_20__["default"].init();
});
...
window.addEventListener('dialogOpen', event => {
  ...
  if (dialogId === 'storiesDialog') {
    ...
    new _Stories__WEBPACK_IMPORTED_MODULE_24__["default"](storiesElement, {
...
function AllPages() {
  ...
  document.querySelectorAll('[data-js-card-splide]').forEach(cardSlider => {
    const cardSliderInstance = new _splidejs_splide__WEBPACK_IMPORTED_MODULE_0__["default"](cardSlider);
